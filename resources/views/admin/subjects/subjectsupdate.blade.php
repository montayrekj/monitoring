    ...
    <body style="background: #eee;">
        ...
        <div class="container-fluid" style="background:#F5FCFF; height: 100%; padding: 0px;">
            ...
            <div class="content" style="width: 80%; height: 100%; float: right; padding: 20px; display: flex; flex-direction: column; overflow: scroll">
                <form action="/admin/subjects/update/{{$subject->subjectId}}" method="POST">
                    @csrf
                    <div class="form-group" >
                        <label for="subjectname">Subject Name</label>
                        <input type="text" class="form-control" id="subjectname" name="subjectname" placeholder="Enter section name" value="{{$subject->name}}">
                    </div>
                    <div class="form-group" >
                        <label for="subjectadviser">Section Teacher</label>
                        <select class="form-control" id="subjectadviser" name="subjectadviser">
                            @for($counter = 0; $counter < $teachers->count(); $counter++)
                                @php
                                    if($teachers[$counter]->id == $subject->teacherId) {
                                        $selected = 'selected';
                                    } else {
                                        $selected = '';
                                    }
                    ...
                    <div style="padding-top: 10px;">
                        <button type="submit" class="btn btn-primary" style="width: 200px;">Update Subject</button>
                    </div>
                </form>
            </div>
        </div>
